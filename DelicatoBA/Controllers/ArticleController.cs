        ...
        public ActionResult Article(InsertArticleViewModel model, FormCollection fc)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isPost)
                {
                    ...
                    var article = _unitOfWork.ArticleRepository.GetQuery().AsNoTracking();
                    if (article.Any(p => p.Url.ToLower().Trim() == model.Article.Url.ToLower().Trim()))
                    {
